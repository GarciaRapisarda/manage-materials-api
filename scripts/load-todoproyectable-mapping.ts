...
import { CATEGORY_MAP } from "./todoproyectable-category-map";

const MAPPING_JSON = path.join(
  __dirname,
  "todoproyectable-category-mapping.json"
);
...
export function loadTodoProyectableMapping(): Record<string, string> {
...
function loadMappingFromCsv(csvPath: string): Record<string, string> {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!slug) continue;
    map[slug] = myCategory || (CATEGORY_MAP[slug] ?? "Herramientas");
  }
