...
import { CATEGORY_MAP } from "./ropelato-category-map";

const MAPPING_JSON = path.join(__dirname, "ropelato-category-mapping.json");
const MAPPING_CSV = path.join(
  process.cwd(),
  "scripts",
  "output",
  "ropelato-category-mapping.csv"
);

export function loadRopelatoMapping(): Record<string, string> {
...
function loadMappingFromCsv(csvPath: string): Record<string, string> {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!slug) continue;
    map[slug] = myCategory || (CATEGORY_MAP[slug] ?? "");
  }
